...
def check(config_path, train_path, test_path, ref_path, num_clients, num_classes, alpha, niid=False, 
...
def save_file(config_path, train_path, test_path, train_data, test_data, ref_data, num_clients, 
                num_classes, statistic,alpha, ref_path = None, niid=False, balance=True, partition=None):
    config = {
    ...
    for idx, train_dict in enumerate(train_data):
        with open(train_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=train_dict, protocol=4)
    for idx, test_dict in enumerate(test_data):
        with open(test_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=test_dict, protocol=4)
    if ref_data != []:
        for idx, ref_dict in enumerate(ref_data):
            with open(ref_path + str(idx) + '.npz', 'wb') as f:
                np.savez_compressed(f, data=ref_dict, protocol=4)
